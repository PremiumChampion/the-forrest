...
#define ADC_CONVERTER_TRANSISTOR_PIN 30
    ...
    gpio_result init()
    {
        ...
        ret = gpio_pin_configure(gpio_dev, ADC_CONVERTER_TRANSISTOR_PIN, GPIO_OUTPUT);
        if (ret < 0)
        {
            LOG_ERR("Failed to configure ADC converter transistor pin\n");
            return ERROR;
        }
    ...
    gpio_result set(gpio_pin pin, gpio_state value)
    {
        ...
        {
            return gpio_pin_set(gpio_dev, ADC_CONVERTER_TRANSISTOR_PIN, value == HIGH ? 1 : 0) == 0 ? OK : ERROR;
        }
    ...
    gpio_result get(gpio_pin pin, gpio_state &value)
    {
        ...
        if (pin == ADC_CONVERTER_TRANSISTOR)
        {
            return gpio_pin_get(gpio_dev, ADC_CONVERTER_TRANSISTOR_PIN) == 0 ? OK : ERROR;
        }
